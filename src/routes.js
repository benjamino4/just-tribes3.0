...
// ---- economy constants (server-authoritative) ----
const ASH_MINUTES = 30, ASH_CAP = 3, ASH_UNIT = 90;
const FOUND_EMBER = 25000, FOUND_MEMBERS = 1;
const QUEST = { cry:120, feed:250, stoke:300, invite:500, ash:180 };
...
router.get('/state', async (req,res,next)=>{ try{
  ...
  const elapsed = Math.max(0, now - (readyAt - ASH_MINUTES*60000));
  let ashPending = Math.min(ASH_CAP, Math.floor(elapsed/(ASH_MINUTES*60000)));
  ...
  res.json({
    user: fresh, tribe, ashPending, ashUnit: ASH_UNIT,
    config: { ashMinutes:ASH_MINUTES, ashCap:ASH_CAP, foundEmber:FOUND_EMBER, foundMembers:FOUND_MEMBERS },
...
router.post('/checkin', async (req,res,next)=>{ try{
  ...
  const reward = QUEST.feed + Math.min(1000, streak*40);
...
router.post('/ash/collect', async (req,res,next)=>{ try{
  ...
  const elapsed = u.ash_ready_at ? Math.max(0, now - (readyAt - ASH_MINUTES*60000)) : 0;
  const units = Math.min(ASH_CAP, Math.floor(elapsed/(ASH_MINUTES*60000)));
...
router.post('/tribe/create', async (req,res,next)=>{ try{
  ...
  if (Number(u.ember) < FOUND_EMBER) return res.status(400).json({error:'not enough Ember', need:FOUND_EMBER});
  ...
  await q('UPDATE users SET tribe_id=$1, ember=ember-$2, role=$3 WHERE id=$4',[t.id,FOUND_EMBER,'Chief',u.id]);
...
router.post('/tribe/donate', async (req,res,next)=>{ try{
  ...
  await addLoyalty(u, Math.floor(amt/50));
...
router.post('/war/declare', async (req,res,next)=>{ try{
  ...
  const war = (await q(`INSERT INTO wars(attacker_id,defender_id,challenge_id,goal,metric,stake_pct,reward_ember,attacker_start,defender_start,end_at)
     ...
     [u.tribe_id, foe.id, c.id, c.goal, c.metric, c.stake, c.reward, aStart, dStart, String(c.days)])).rows[0];
