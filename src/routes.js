// ---------------------------------------------------------------------------
// REST API for TRIBES. Phase 1: trials, kiva (SSE), bonfires, cosmetics,
// starter bundle, tribe levels + names, gift-a-tribe-upgrade.
// ---------------------------------------------------------------------------
...
import { CFG, PALETTES, BANNERS, parseJSON } from './config.js';
...
router.get('/state', rateLimit('state', 120), async (req,res,next)=>{ try{
  ...
  // Ash preview — same semantics as before (ash_ready_at = last collection + N*ashMinutes)
  const now = Date.now();
...
router.get('/trials', rateLimit('trials', 60), async (req,res,next)=>{ try{
  const all = await listTrials(true);
  res.json({ trials: all });
}catch(e){ next(e); }});
...
router.post('/tribe/upgrade', rateLimit('upgrade', 6), async (req,res,next)=>{ try{
  ...
  if (!['Chief','Head','Elder'].includes(u.role)) return res.status(403).json({ error:'only Elders, Heads, or the Chief can raise the level' });
  ...
  const nextCost = Number(costs[cur]); // cost to go from cur -> cur+1
...
// SSE — mounted before the JSON body parser path (see index.js comment)
export function kivaSse(req,res){
  // called from index.js as a separate route so we can skip express.json body parse
...
router.post('/war/declare', rateLimit('war', 6), async (req,res,next)=>{ try{
  ...
  const war = (await q(
    `INSERT INTO wars (attacker_id,defender_id,challenge_id,goal,metric,stake_pct,reward_ember,attacker_start,defender_start,end_at)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9, now() + ($10 || ' days')::interval)
     RETURNING *`,
    [u.tribe_id, foe.id, c.id, c.goal, c.metric, stake, reward, aStart, dStart, String(c.days)]
  )).rows[0];
  const foeName = (await q('SELECT name FROM tribes WHERE id=$1', [foe.id])).rows[0]?.name || 'a rival';
  await Kiva.postMessage(u.tribe_id, u.id, `⚔️ War declared against ${foeName}.`, 'war');
