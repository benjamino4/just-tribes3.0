// ---------------------------------------------------------------------------
// Tribe War engine: challenge catalogue, matchmaking, scoring, resolution.
// A war picks a RANDOM challenge; both tribes race the same goal over a window.
// Winner earns Ember + forces the loser to pay TRIBUTE (a % of their Pyre).
// ---------------------------------------------------------------------------
import { q } from './db.js';

// metric = column on `tribes` that is snapshotted at war start; score = delta.
export const CHALLENGES = [
  ...
  { id:'quest_frenzy',  name:'Quest Frenzy',     glyph:'\u2694\uFE0F', metric:'quests_total',   min:25,   max:90,   days:5, stake:20, reward:5500,
    desc:'Complete the most daily quests across the whole tribe.' },
...
// refresh live scores for an active war row
export async function refreshWar(war){
  ...
  const as = Math.max(0, a - Number(war.attacker_start));
  const ds = Math.max(0, d - Number(war.defender_start));
  await q('UPDATE wars SET attacker_score=$1, defender_score=$2 WHERE id=$3', [as, ds, war.id]);
  war.attacker_score = as; war.defender_score = ds;
  return war;
}

// decide winner: blitz -> first past goal; else higher score at end (goal caps early win)
export async function maybeResolve(war){
  ...
  const aHit = as >= goal, dHit = ds >= goal;
  let done = ended;
  if (!done && (aHit || dHit)) done = true;                 // someone reached the goal
  if (!done) return war;

  let winnerId = null;
  if (as === ds) winnerId = null;                            // draw
  else winnerId = as > ds ? war.attacker_id : war.defender_id;
  ...
  if (winnerId){
    ...
    // move tribute + mint reward to winner; wins/losses
    await q('UPDATE tribes SET treasury = treasury - $1, losses = losses + 1 WHERE id=$2', [tribute, loserId]);
    await q('UPDATE tribes SET treasury = treasury + $1 + $2, wins = wins + 1 WHERE id=$3', [tribute, war.reward_ember, winnerId]);
    await q(`INSERT INTO ledger(tribe_id,kind,detail,ember) VALUES ($1,'war_tribute',$2,$3),($4,'war_reward',$5,$6)`,
      [loserId, 'Paid tribute after defeat', -tribute, winnerId, 'Spoils of war + tribute', tribute + Number(war.reward_ember)]);
  }
  await q('UPDATE wars SET status=$1, winner_id=$2, tribute=$3, resolved_at=now() WHERE id=$4',
    ['resolved', winnerId, tribute, war.id]);
